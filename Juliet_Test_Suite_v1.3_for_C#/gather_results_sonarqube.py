...
def gather_results(root: Path) -> List[Tuple[str, str, str, str]]:
    ...
    sonarqube_csv_fields = ['Name', 'Description', 'Severity', 'Message', 'File', 'Start line', 'Start column', 'End line', 'End column']
    all_errors = []
    
    
    for i, n in enumerate(root.glob('**/SQresults.csv')):
        logging.debug(f'reading ({i}): {n}')

        with open(n, 'r') as f:
            findings = csv.DictReader(f, fieldnames=sonarqube_csv_fields)
            processed = [
...
if __name__ == '__main__':
    ...
    logging.debug(f"{real_positives}/{true_positives_errors} real positives, {real_negatives}/{true_negatives_errors} real negatives")
